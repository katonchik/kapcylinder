<h3 class="page_title">Дух игры: <a href="<?php echo SITE_URL; ?>/spirit/">Сводная статистика</a> / <a href="<?php echo SITE_URL; ?>/spirit_standard/">Стандарт</a> / Easy level / <a href="<?php echo SITE_URL; ?>/spirit_centered/">Центрированный</a></h3>
...
if (isset($key) && (in_array($key, $s) || in_array(str_replace (" DESC", "", $key), $s)))
{
	...
	WHERE s1.receiver <> s2.receiver AND  s1.game_id = s2.game_id AND s1.receiver = t1.team_id AND s2.receiver = t2.team_id AND s1.game_id=uo_game.game_id AND s2.mvp=p1.player_id AND s2.msp=p2.player_id AND s2.mpp=p3.player_id AND s2.receiver > 1016
