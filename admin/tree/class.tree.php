...
//		'data_table'		=> 'structure',		// table for additional fields (apart from structure ones, can be the same as structure_table)
		'data_table'		=> 'data',		// table for additional fields (apart from structure ones, can be the same as structure_table)
	...
	public function get_node_by_page_url($page_url, $options = array()) {
		$node = $this->db->one("
			SELECT 
				s.".implode(", s.", $this->options['structure']).", 
				d.".implode(", d.", $this->options['data'])." 
		...
		if(isset($options['with_children'])) {
			$node['children'] = $this->get_children($node['id'], isset($options['deep_children']));
		}
		if(isset($options['with_path'])) {
			$node['path'] = $this->get_path($node['id']);
		}
